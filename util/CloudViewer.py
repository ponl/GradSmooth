...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Displays a 3d Point Cloud")
    parser.add_argument('cloud_path', metavar='cloud_path', type=str,
                        help="Input file for the point cloud.")
    args = parser.parse_args()

    try:
        cloud = np.load(args.cloud_path)
    except:
        logging.exception("Failed to open point cloud: {}".format(args.cloud_path))

    if len(cloud.shape) != 2 or cloud.shape[1] != 3:
        logging.error("This point cloud has an unacceptable shape.")
        sys.exit(0)
